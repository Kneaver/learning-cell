    ...
    LRS.prototype = {
                ...
                if (isXD) {
                    if (env.hasCORS) {
                        if (env.useXDR && schemeMatches) {
                            ...
                            if (cfg.allowFail) {
                                if (this.alertOnRequestFailure) {
                                    this.log("[warning] LRS invalid: cross domain request for differing scheme in IE with XDR");
                                }
                            }
                            else {
                                if (this.alertOnRequestFailure) {
                        ...
                        if (cfg.allowFail) {
                            if (this.alertOnRequestFailure) {
                                this.log("[warning] LRS invalid: cross domain requests not supported in this browser");
                            }
                        }
                        else {
                            if (this.alertOnRequestFailure) {
                    ...
                    if (httpStatus === undefined || (httpStatus >= 200 && httpStatus < 400) || notFoundOk) {
                        ...
                        if (httpStatus > 0) {
                            ...
                            if (self.alertOnRequestFailure) {
                                console.log("[warning] There was a problem communicating with the Learning Record Store. (" + httpStatus + " | " + xhr.responseText+ ")");
                            }
                            if (cfg.callback) {
        ...
        */
        _queryStatementsRequestCfg: function (cfg) {
            ...
            // and potentially this.log the user
            //
            for (prop in cfg.params) {
                if (cfg.params.hasOwnProperty(prop)) {
